...
int read_card(int pages[4][45], int *card_detected) {
    *card_detected = 0;
    if (!ctx)
        return -1;
    ...
    *card_detected = 1;
    ...
    /*if (mifare_ultralightc_authenticate(tag, mfu_key) < 0 &&
            mifare_ultralightc_authenticate(tag, default_key) < 0){
    */
