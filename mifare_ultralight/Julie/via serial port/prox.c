...
// static void relay_loop(serial_port sp1, serial_port sp2) {
//     uint8_t buffer[2048];
//     uint32_t len;
...
//         if (uart_receive(sp1, buffer, sizeof(buffer), &len) == WN_SUCCESS && len > 0) {
//             uart_send(sp2, buffer, len);
//             printf("Tag >> %u octets >> Lecteur\n", len);
//         }
        
//         // Sens : Lecteur (Proxy) -> Tag (Mole)
//         if (uart_receive(sp2, buffer, sizeof(buffer), &len) == WN_SUCCESS && len > 0) {
...
//     printf("[+] Commandes d'initialisation envoyées.\n");
//     Sleep(500); // Laisse le temps au firmware de changer d'état
//     printf("[+] Lancement du pont relay...\n");
...
int main(int argc, char *argv[]) {
    //main_bis();
    //sleep(10000); // Laisse le temps aux messages de s'afficher avant de quitter
    pm3_init();
