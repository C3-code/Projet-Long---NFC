...
# read_card
Blocks64 = c_uint8 * 16 * 64
lib.read_card.argtypes = [POINTER(Blocks64), POINTER(c_int)]
...
def read_card():
    ...
    blocks = Blocks64()
    detected = c_int(0)
    res = lib.read_card(byref(blocks), byref(detected))
    # Conversion en liste Python
    py_blocks = [[blocks[b][i] for i in range(16)] for b in range(64)]
